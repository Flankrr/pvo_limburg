...
with open("experiments/all_articles_sme_ex.json", "r", encoding="utf-8") as f:
    data = json.load(f)

# We filter out the articles that are not from the region before pre-processing.
df = build_geo_df("experiments/all_articles_sme_ex.json", min_conf=0.6)
...
train_df.to_json(
    "experiments/all_articles_filtered_sme_ex.json",
    orient="records",
    indent=2,
    force_ascii=False,
)
...
output_file = "experiments/all_articles_top20_keywords_sme_ex.json"
